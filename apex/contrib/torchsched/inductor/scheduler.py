...
class MultiCudaStreamScheduler(Scheduler):
    ...
    def schedule_multi_cuda_streams(self) -> None:
        ...
        schedule_log.debug(f"{'Multi-CUDA-Stream scheduling results'.ljust(79, '=')}")
    ...
    def get_final_events_to_sync(self) -> set[CudaEventSym]:
        ...
        if self.unjoined_events.get(DEFAULT_STREAM_IDX):
            raise ValueError(
                f"Unexpected {self.unjoined_events} DEFAULT_STREAM_IDX={DEFAULT_STREAM_IDX} on default stream",
            )
        events_to_sync = set()
